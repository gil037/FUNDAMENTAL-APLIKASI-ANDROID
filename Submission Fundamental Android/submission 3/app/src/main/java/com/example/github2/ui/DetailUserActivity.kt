...
import com.example.github2.utils.SnackBarUtil
...
class DetailUserActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var isChecked = false
        val count = detailUserViewModel.checkUserFavorite(id!!)
        isChecked = count > 0
        binding.favoriteUserFloatingButton.setOnClickListener {
            isChecked = !isChecked
            if(isChecked) {
                detailUserViewModel.addUserFavorite(login, id, avatar)
                SnackBarUtil.showSnackBar(this, getString(R.string.favorite))
            } else {
                detailUserViewModel.removeUserFavorite(id)
                SnackBarUtil.showSnackBar(this, getString(R.string.unfavorite))
            }
        }
    }
